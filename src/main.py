...
async def index(Authorization: Annotated[str | None, Header()]) -> JWTValidity:
    token_headers = JWTDecoder(
        token=Authorization).get_token_headers()  # Extract token headers
    ...
    # Using await async due to fetching the validation from an external source (x5u).
    ExpirationValidator(
